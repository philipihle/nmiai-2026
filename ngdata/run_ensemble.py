...
def detect_with_session(img, session, input_name, conf, iou, imgsz):
...
def detect_ensemble(img, sessions, input_names, conf, iou, imgsz):
    all_boxes, all_scores, all_class_ids = [], [], []

    for session, input_name in zip(sessions, input_names):
        boxes, scores, class_ids = detect_with_session(img, session, input_name, conf, iou, imgsz)
        if len(boxes) > 0:
            all_boxes.append(boxes)
            all_scores.append(scores)
            all_class_ids.append(class_ids)
...
def run(input_path: Path, output_path: Path, weights_path: Path,
    ...
    # Load primary model
    session1 = ort.InferenceSession(str(weights_path), providers=providers)
    sessions.append(session1)
    input_names.append(session1.get_inputs()[0].name)
    print(f"Model 1 loaded: {weights_path.name}")

    # Load secondary model if present
    model2_path = base / "model2.onnx"
    if model2_path.exists():
        session2 = ort.InferenceSession(str(model2_path), providers=providers)
        sessions.append(session2)
        input_names.append(session2.get_inputs()[0].name)
        print("Model 2 loaded (ensemble active)")

    # Warmup
    _dummy = np.zeros((1, 3, imgsz, imgsz), dtype=np.float32)
    for sess, iname in zip(sessions, input_names):
    ...
    for img_path in images:
        ...
        try:
            boxes, scores, class_ids = detect_ensemble(img, sessions, input_names, conf, iou, imgsz)
        except Exception:
            continue
