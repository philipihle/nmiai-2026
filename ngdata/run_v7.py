...
CLASSIFIER_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32).reshape(3, 1, 1)
CLASSIFIER_STD = np.array([0.229, 0.224, 0.225], dtype=np.float32).reshape(3, 1, 1)
...
def infer_image(img, session, input_name, conf, imgsz):
...
def detect(img, session, input_name, conf, iou, imgsz):
    all_boxes, all_scores, all_class_ids = infer_image(img, session, input_name, conf, imgsz)
...
def classify_crops(img, boxes, scores, cls_session, cls_input_name, yolo_class_ids,
                   reclassify_threshold=0.70):
    """Reclassify low-confidence YOLO detections using fine-tuned classifier."""
    if len(boxes) == 0:
        return yolo_class_ids.copy()

    new_class_ids = yolo_class_ids.copy()
    h, w = img.shape[:2]
    crops = []
    crop_indices = []

    for i in range(len(boxes)):
        if scores[i] >= reclassify_threshold:
            continue
        ...
        crop_resized = cv2.resize(crop_rgb, (CLASSIFIER_IMG_SIZE, CLASSIFIER_IMG_SIZE),
                                   interpolation=cv2.INTER_LINEAR)
        blob = crop_resized.astype(np.float32) / 255.0
        blob = blob.transpose(2, 0, 1)
        blob = (blob - CLASSIFIER_MEAN) / CLASSIFIER_STD
        crops.append(blob)
    ...
    FIXED_BATCH = 32
    for start in range(0, len(crops), FIXED_BATCH):
        chunk = crops[start:start + FIXED_BATCH]
        n = len(chunk)
        batch = np.zeros((FIXED_BATCH, 3, CLASSIFIER_IMG_SIZE, CLASSIFIER_IMG_SIZE), dtype=np.float32)
        batch[:n] = np.stack(chunk)
        logits = cls_session.run(None, {cls_input_name: batch})[0][:n]
        predicted_classes = logits.argmax(axis=1)
        for j, idx in enumerate(crop_indices[start:start + n]):
            new_class_ids[idx] = predicted_classes[j]
...
def run(input_path: Path, output_path: Path, weights_path: Path,
    ...
    cls_path = base / "classifier_finetuned.onnx"
    use_classifier = cls_path.exists()
    if use_classifier:
        ...
        print(f"Fine-tuned classifier loaded")
        # Warmup
        _dummy_cls = np.zeros((32, 3, CLASSIFIER_IMG_SIZE, CLASSIFIER_IMG_SIZE), dtype=np.float32)
        cls_session.run(None, {cls_input_name: _dummy_cls})
    else:
        print("Classifier not found, using YOLO classes only")
    ...
    for img_path in images:
        ...
        if use_classifier and len(boxes) > 0:
            try:
                class_ids = classify_crops(img, boxes, scores, cls_session, cls_input_name,
                                           class_ids)
            except Exception:
                pass
