...
def classify_crops(img, boxes, scores, cls_session, cls_input_name, yolo_class_ids,
    ...
    """Reclassify low-confidence detections using fine-tuned classifier."""
    if len(boxes) == 0:
...
def run(input_path: Path, output_path: Path, weights_path: Path,
    ...
    cls_path = base / "classifier_realcrops.onnx"
    ...
    for img_path in images:
        ...
        if use_classifier and len(boxes) > 0:
            try:
                class_ids = classify_crops(img, boxes, scores, cls_session, cls_input_name, class_ids)
            except Exception:
                pass
